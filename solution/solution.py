...
OPENAI_MODEL = os.getenv("LAB_MODEL", "gemini-2.5-pro")
...
def compare_models(prompt: str) -> dict:
    ...
    cost = (len(gpt4o_resp.split()) / 0.75) / 1000 * pricing["output"]
...
def run_assistant(
    ...
    while True:
        ...
        turn_cost = estimate_cost(prompt_text, full_reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Lat':<10} | {'Mini Lat':<10}"
    lines = [header, "-" * len(header)]
    for r in results:
        p = r["prompt"][:37] + "..." if len(r["prompt"]) > 40 else r["prompt"]
        g_res = str(r.get("gpt4o_response", ""))
        m_res = str(r.get("mini_response", ""))
        g_res = g_res.replace("\n", " ")
        m_res = m_res.replace("\n", " ")
        g_res = g_res[:37] + "..." if len(g_res) > 40 else g_res
        m_res = m_res[:37] + "..." if len(m_res) > 40 else m_res
        g_lat = f"{r.get('gpt4o_latency', 0):.2f}"
        m_lat = f"{r.get('mini_latency', 0):.2f}"
        lines.append(f"{p:<40} | {g_res:<40} | {m_res:<40} | {g_lat:<10} | {m_lat:<10}")
    return "\n".join(lines)
